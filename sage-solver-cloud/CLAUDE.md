...
4. Read `.build/Claude-AI_Claude-Code-ClickUp-Groot.md` — ClickUp coordination protocol
...
Then report:
> "Resuming Project Groot. Last session: [last BUILD_LOG entry summary]. Current phase: [phase]. Ready to continue with: [next action]."
...
Groot is a domain-agnostic LLM runtime environment (FastAPI + SQLite + React). External LLM agents call Groot tools over MCP or REST HTTP to store artifacts, register live React pages, and run domain-specific tools. The LLM is always external — Groot never embeds a model. `sage/` is the first Groot app module.
...
| Groot phase gates | Groot workflow | `901113373077` |
| Session logs | Activity Log | `901113365508` |
| Async comms | Chat | `901113365507` |

**ClickUp space/folder:**
- Space: Projects (`90030426535`) → Folder: Groot (`90117770941`)
...
Never begin a phase until the previous phase is verified and Peter has approved the gate task in Groot workflow (`901113373077`).

---

## Hard rules (never violate)

- No code in `groot/` that knows about domains (optimization, translation, etc.)
- No LLM API calls from inside Groot runtime
...
- Do not modify `groot_spec.md` (Peter's original vision doc)
